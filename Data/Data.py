...
def interval_to_timedealta(interval):
    dict_ = {"1m": timedelta(minutes=1), "5m": timedelta(minutes=5), "10m": timedelta(minutes=10), "15m": timedelta(minutes=15),
             "30m": timedelta(minutes=30), "1h": timedelta(hours=1), "2h": timedelta(minutes=2), "4h": timedelta(hours=4)}
...
"""
class SwapData:
    save_path = r"D:\quant_research\Crypto因子研究\Data"
    BASE_URL = "https://fapi.binance.com"
    REQ_LIMIT = 1000
    SUPPORT_INTERVAL = {"1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h", "6h", "8h", "12h", "1d", "3d", "1w", "1M"}


    def __init__(self, swap = "BTCUSDT", output = True,
    ...
    def GetData(self):
        ...
        if "klines" in self.datalst:
            df_klines = self.download_full_klines(start=self.start, end=self.end, req_interval=self.req_interval)
            df_klines["timestamp"] = df_klines["timestamp"].apply(lambda x: adjust_timestamp(x))
            self.output_dict["klines"] = df_klines
            self.output_dict["klines"]["symbol"] = self.crypto
            self.output_dict["klines"].set_index(keys=["symbol", "timestamp"], inplace=True)
        if "fundingRate" in self.datalst:
            self.output_dict["fundingRate"] = self.download_full_fundingRate(start=self.start, end=self.end, req_interval=self.req_interval)
            self.output_dict["fundingRate"]["symbol"] = self.crypto
            self.output_dict["fundingRate"].set_index(keys=["symbol", "timestamp"], inplace=True)
        if "globalLongShortAccountRatio" in self.datalst:
            self.output_dict["globalLongShortAccountRatio"] = self.download_full_globalLongShortAccountRatio(start=self.start, end=self.end, req_interval=self.req_interval)
            self.output_dict["globalLongShortAccountRatio"]["symbol"] = self.crypto
        ...
        if "takerlongshortRatio" in self.datalst:
            self.output_dict["takerlongshortRatio"] = self.download_full_takerlongshortRatio(start=self.start, end=self.end, req_interval=self.req_interval)
            self.output_dict["takerlongshortRatio"]["symbol"] = self.crypto
            self.output_dict["takerlongshortRatio"].set_index(keys=["symbol", "timestamp"], inplace=True)
        if "openInterestHist" in self.datalst:
            self.output_dict["openInterestHist"] = self.download_full_openInterestHist(start=self.start, end=self.end, req_interval=self.req_interval)
            self.output_dict["openInterestHist"]["symbol"] = self.crypto
            self.output_dict["openInterestHist"].set_index(keys=["symbol", "timestamp"], inplace=True)

    # 获取k线数据, 默认是永续合约数据
    @retry(reraise=True, stop=stop_after_attempt(max_attempt_number=10), wait=wait_fixed(5), retry=retry_if_exception(Exception))
    ...
    def download_full_klines(self, start, limit = 1500, end=None, req_interval=None):
        data_save_path = os.path.join(SwapData.save_path, self.crypto, "continuousKlines", self.interval)
            ...
            try:
                ...
                if not df_existing.empty:
                    ...
                    if not download_ranges:
                        print("K线数据已完整，无需下载")
                        df_existing = df_existing.loc[(df_existing["timestamp"] <= end) & (df_existing["timestamp"] >= start)]
                        return df_existing
        ...
        for (start_ts, end_ts) in tqdm(start_end_pairs, desc="K线数据正在下载："):
            tmp_kline = self.get_klines(self.crypto.replace("/", ""), self.interval, since=start_ts, limit=limit,
                                        to=end_ts)
            if len(tmp_kline) > 0:
        ...
        df["timestamp"] = df["timestamp"].apply(lambda x:time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(x)/1000)))
    ...
    def download_full_fundingRate(self,  start, end=None,  req_interval=None):
        data_save_path = os.path.join(SwapData.save_path, self.crypto, "fundingRate", self.interval)
        ...
        FundingRates["fundingTime"] = FundingRates["fundingTime"].apply(lambda x:time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(x)/1000)))
    ...
    def download_full_takerlongshortRatio(self, start, limit = 500, end=None, req_interval=None):
        data_save_path = os.path.join(SwapData.save_path, self.crypto, "longshortRatio", self.interval)
        ...
        longshortRatios["timestamp"] = longshortRatios["timestamp"].apply(lambda x:time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(x)/1000)))
    ...
    def download_full_globalLongShortAccountRatio(self, start, end=None,  req_interval=None, limit = 500):
        data_save_path = os.path.join(SwapData.save_path, self.crypto, "LongShortAccountRatio", self.interval)
        ...
        for (start_ts, end_ts) in tqdm(start_end_pairs, desc="多空持仓人数比数据正在下载："):
            LongShortAccountRatio = self.get_globalLongShortAccountRatio(interval=self.interval, since=start_ts, limit=limit, to=end_ts)
        ...
        LongShortAccountRatios["timestamp"] = LongShortAccountRatios["timestamp"].apply(lambda x:time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(x)/1000)))
    ...
    def download_full_openInterestHist(self, start, end=None,  req_interval=None, limit = 500):
        data_save_path = os.path.join(SwapData.save_path, self.crypto, "openInterestHist", self.interval)
        ...
        openInterestHists["timestamp"] = openInterestHists["timestamp"].apply(lambda x:time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(x)/1000)))
...
if __name__ == "__main__":
    ...
    Swap_1 = SwapData(swap = "SOLUSDT", interval = '1m', datalst = ["klines"], start = "2021-01-01 00:00:00", end = "2022-01-01 00:00:00")
    Swap_1.GetData()
